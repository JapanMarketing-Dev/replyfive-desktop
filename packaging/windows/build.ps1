...
dotnet publish (Join-Path $root "src\ReplyFive.Desktop\ReplyFive.Desktop.csproj") -c Release -f net10.0-windows10.0.19041.0 -r win-x64 --self-contained true `
...
if (-not $SkipInstaller) {
  ...
  & $iscc "/DAppVersion=$Version" "/DPublishDir=$publish" (Join-Path $PSScriptRoot "ReplyFive.iss")
  if ($LASTEXITCODE -ne 0) { throw "ISCC failed" }
  if ($SignCommand) { Invoke-Expression ($SignCommand -replace "\{file\}", (Join-Path $dist "ReplyFive-Setup.exe")) }
  Copy-Item (Join-Path $dist "ReplyFive-Setup.exe") (Join-Path $dist "ReplyFive-Setup-$Version.exe") -Force
}
...
if (-not $SkipMsix) {
  ...
  & $makeappx pack /d (Join-Path $msixRoot "app") /p (Join-Path $dist "ReplyFive-$Version.msix") /o
